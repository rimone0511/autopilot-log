...
This file names **stop / skip / park** walls. It is not a payment plan, not a wallet dump, and not a bid. This CU pass ends at a **profile draft** ([PLAYBOOK.md](PLAYBOOK.md)).
...
Related: [STOP-KYC.md](STOP-KYC.md) · [FIELD-MAP.md](FIELD-MAP.md) · [STATUS.md](STATUS.md)

---

## Do not invent

Do **not** write into git, chat, STATUS, or a later bid letter:
...
|---|---|---|---|
| **Minimum Account Balance / Site wallet** | Bid UI asks you to add funds, hold a balance, or “deposit to continue” before Bid | **STOP.** Do not top up. Do not buy credit. Hand the **screen type** (not the amount) to morning human. Outcome `wallet_stop` | Live balance, required minimum, reserved funds |
...
| **Membership / Corporate** | Free vs paid plan upsell, extra bids, extra skill slots, cover photo, extra profiles | Stay **free**. Close the upsell. If the control you need is paid-only, **stop** (`card_wall` if it demands a card) | Plan prices, “extra bids per month” counts |
...
| **Empty required rate** | Form blocks save/submit until bid amount or hourly is filled | Park `rate_empty` / `rate_required`. Stop. Do not send | Invented USD |
| **Hourly / bid / delivery fields** | Number fields next to the proposal | Local ledger placeholders only: `{{HOURLY_RATE_USD}}` `{{BID_AMOUNT_USD}}` `{{DELIVERY_DAYS}}` — empty in git; **not used this pass** | Live quotes in fenced paste |
...
| **This CU pass: Bid button** | Place Bid / Submit proposal | **Do not click.** Surgical bids = later human GO | “We still have leftover bids” |

User Agreement (https://www.freelancer.com/about/terms): before submitting a bid, a Seller must hold the **Minimum Account Balance** specified on the live Fees & Charges schedule. This pack records that the **rule exists**. It does **not** copy the amount. If the bid UI asks you to fund that balance: **stop**. This pass should never reach that UI.
...
- Contest **entry** may be described as free; a contest fee applies on award. This pack still **does not enter contests.**
...
- Park `rate_empty` when the live form demands a number you will not invent
- Stop at wallet funding / KYC / paid upgrade / contest
- **Do not Bid** on this CU pass
...
- Treating leftover monthly bids as a reason to spray

## 日本語（運用だけ）

残高・残り入札・手数料の数字は捏造しない、git にも書かない。ライブは https://www.freelancer.com/feesandcharges と画面。ウォレット入金・有料入札アップグレード・会員課金・コンテストはこのパックからしない。必須金額欄が空なら `rate_empty` で止める。このパスでは Bid を押さない。
