...
State: **DRAFT_ONLY** / **prep**  
Authoring: logged-out public GET only. **No signup. No OAuth. No POST. No cookies saved.**

This is a serial **prep box**, not a live CU log and not an activity-gate re-judge. Sibling gates already marked these desks `pass` (PR#12 JP Wave B except Workshift; PR#13 for Workshift). This GET only re-cites public URLs so CU has a current pointer.
...
Current rows (authoring time): **all six** `pack_ready` + `pending`.

Wave B live CU still waits on Wave A ([PR#54](https://github.com/rimone0511/autopilot-log/pull/54)). Do not treat this STATUS as a signup GO.
...
|---|---|---|---|---|---|---|---|
| 1 | B02 | https://goworkship.com/ | `/portal/search` 200. Headings include リモ可 / LLM 業務効率化カード. **No card `2026-09-*` date in this HTML** (PR#12 used this listing as `pass`; this GET does not add a date) | `/signup` 200「フリーランス登録をする」「SNSで登録」+ FirebaseUI. Page JS includes `GoogleAuthProvider.PROVIDER_ID` | 前払いヘルプに％なし → 作らない | Not enterprise. Confirm Google **icon label** before click | `pending` |
| 2 | B01 | https://sokudan.work/ | Home 200. Embedded `contractType: outsourcing` + `createdAt` **2026-09-15**. Titles include 基本リモ FDE | `/signup/pro` 200「無料新規登録」`alt="Google で登録"` → `/users/auth/google?category=signup` | 規約に「手数料」語のみ、％なし → 作らない | Not 企業. Facebook「推奨」無視 | `pending` |
| 3 | B19 | https://offers.jp/ | `/jobs/engineer/side-job` 200. 業務委託カード「【フルリモート】AI×FDE…」更新日文字列 **2026-09-10** (also 09-08 / 09-07). `/signup` **404** | `/worker/signup` 200. `data-testid="auth-google"` → `/oauth/worker_signup/google`. Heading「メールアドレスで登録する」 | 規約: マッチング成功報酬はクライアント側の語. ユーザー％ **unstated** → 作らない | Not `/client/`. Do not 応募 | `pending` |
| 4 | B06 | https://www.skill-shift.com/ | `GET /api/jobs` 200. Leading `created_at` **2026-09-14 / 13 / 12 / 11**, `is_recruiting: true`. Example position「AI活用で業務効率化！業務棚卸しから始めるAIアドバイザー」（オンライン想定）. `skillshift.jp` **DNS fail** | `/sign-up` 200 SPA shell (meta「個人登録ページ」). Google OAuth **not** in this HTML | ％ **unstated** this GET → 作らない | Email = MAIN mailbox. Facebook 新規禁止. 現場のみ行は応募しない | `pending` |
| 5 | B07 | https://itpropartners.com/ | `/job/sale-4` 200. 最終更新日 **2026/09/08**「コーポレートIT／社内情報システム」、**2026/09/05**「M&A事業部…アポ獲得」 | `/register` 200「あなたの職種を教えてください」（エンジニア / マーケター / デザイナー）. Register HTML has **no** Google signup control (GTM only) | 公式フロー「登録・利用は無料」（sibling). 仲介％ **needs_check** | Agent desk. CU does not attend 面談. Do not apply | `pending` |
| 6 | B09 | https://workshift-sol.com/ | Home 200. Cards link `/jobs/view/13758` etc. That view 200: title SIAL Paris 現地通訳, date string **2026-09-11**,「本人確認: 無し」. **Do not apply that row.** `/jobs/search` 200 + CAPTCHA (listing dates unread there) | `/registration/mail_start` 200. Links: mail_start / `facebook/login` / `linkedin/login` / `github/login`. **No** `/google/login` | ％ **unstated** this GET → 作らない | ≠ Workship. Email = MAIN mailbox. 現地カード回避 | `pending` |
...
1. Do **not** CU-play this serial until Wave A `pending` is parked (PR#54).
2. When Wave B starts: follow [ORDER.md](ORDER.md). One desk. MAIN Google. Draft only. [STOP-KYC.md](STOP-KYC.md).
3. Workship: glance Google icon label on FirebaseUI before click.
4. Workshift: do not apply 現地 cards; do not mix with Workship paste.
5. Keep PR draft. Do not merge until Yuta reviews.
