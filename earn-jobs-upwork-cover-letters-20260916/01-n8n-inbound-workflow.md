...
# JOBS-01 — Upwork: n8n inbound workflow (cover letter)
...
| seller_theme | n8n inbound workflow |
| phase | JOBS |
...
Assumed listing type (not a real job): one inbound n8n path a non-engineer can rerun — trigger, honest steps, destination, failure alert, short SOP.
...
All boxes would have to be true **and** a later human would have to un-park send. This file still does not send.

- [ ] Upwork account is actually usable (today: treat as **may be blocked**; do not retry Google SSO from this pack)
- [ ] Job opened on upwork.com (not a scrape dump)
- [ ] `{{ONE_SPECIFIC_DETAIL}}` is unique to this post
- [ ] Theme still matches **n8n inbound workflow**, not classify-only / approval-gate-only / docs-only
...
```
You mentioned {{ONE_SPECIFIC_DETAIL}} on "{{JOB_TITLE}}". I would treat that as the first n8n path a non-engineer can rerun, not a generic template dump.
```
...
I build n8n workflows with official APIs or documented connectors only: a trigger you already have, honest steps, a destination, retries or a failure alert, and a short SOP (what starts it, what is safe to edit, where secrets live). I do not use browser automation. I do not scrape platforms that have no API.
...
- Title example: `[FICTION] n8n: Typeform to Google Sheet plus Slack only on failure`
...
- Do not send while the Upwork account may be blocked
