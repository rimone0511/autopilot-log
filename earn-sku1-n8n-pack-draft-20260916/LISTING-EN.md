# Gumroad listing paste — SKU-1 (English)

Desk: Gumroad
Product: SKU-1 n8n Inquiry Classifier Pack
...
Do not set a custom permalink live. Keep `{{GUMROAD_PERMALINK_SKU1}}` local.
...
```
Importable n8n stubs that label inbound text (billing / support / sales) and park anything unclear. You send. No auto-reply.
```
...
You import two inactive workflow stubs: one maps inbound text onto labels you already use (billing, support, sales), and one is the hold queue. Unclear messages go to uncertain. The pack does not auto-reply, auto-invoice, or post in your name.
...
AI is optional and off in the stubs. If you later add a model call to suggest a label, you still review it. If you want no AI, leave the stubs as they are.
...
Not included: SKU-0 starter intake (sold separately if/when published), live credentials, guaranteed accuracy, or sending mail for you.

If a stub fails to import on your n8n version, rebuild from the sticky notes on the canvas. See the buyer README for the dummy payload and the operator loop.
```
