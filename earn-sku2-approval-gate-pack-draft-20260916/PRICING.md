# SKU-2 pricing — PLACEHOLDER ONLY
...
## Relationship to SKU-0 and SKU-1

SKU-0 (starter intake) and SKU-1 (inquiry classifier) are **separate** products
and separate price tokens (`{{PRICE_USD_SKU0}}` / `{{PRICE_USD_SKU1}}` live in
those packs, not here). Do not bundle them into SKU-2’s price box in this draft.
