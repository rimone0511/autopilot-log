...
| sample_inventory | 03 |
| cu_serial | CU-15 |
| activity_gate | **needs_check**（`/offers` は SPA 空。案件カード未読） |
| self_serve | **yes**（公開FAQ: Webプロフィールのみ。通常は面談・職務経歴書不要） |
| cu_ready | **true** |
...
| worker_fee_public | アプリFAQ: 「仕事を受けるフリーランス・副業人材の方には一切費用はかかりません。」求人事業者の有料プランは規約第5条。**人材％の数字はFAQに無し → 作らない** |
...
1. **活動** — アプリは生きている。案件一覧 `/offers` はこのGETでシェルのみ。日付なし
2. プロフィールの「検索結果への表示」。FAQは「非公開／公開」から選ぶとある。**公開にしない**。UIラベルはクリック時確認

人が案件を1件開いて日付を見てから CU を進めてよい。見えないまま pass にしない。
...
3. プロフィール入力。表示範囲は **非公開**（または下書き）
4. 応募しない。エージェント仲介案件の職務経歴書が「必須」なら停止

フォールバック: 画面にメール登録がある場合のみ `{{EMAIL}}`。無い画面で Google が死んでいたらこのパックは保留。

規約: 登録には外部SNSアカウント。Googleで始めたら以後 Google。
...
ここまでやってよい: Google登録、メール確認、プロフィール（非公開）。
...
false。2026-09-16 に https://app.any-crew.com/ が HTTP 200。Google ログイン文言あり。
