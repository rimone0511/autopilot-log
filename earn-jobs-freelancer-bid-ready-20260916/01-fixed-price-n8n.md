...
# FL-JOB-01 — Freelancer.com: fixed-price n8n / workflow (bid proposal)
...
If the listing is hourly AI-ops, use [02-hourly-ai-ops.md](02-hourly-ai-ops.md). If it is inbound tag/route only, use [03-lead-classify.md](03-lead-classify.md).
...
4. Form fields (not prose): `{{BID_AMOUNT_USD}}` `{{DELIVERY_DAYS}}` optional `{{MILESTONE_1}}`
...
```
You mentioned {{ONE_SPECIFIC_DETAIL}} on "{{PROJECT_TITLE}}". I would ship that as the first n8n path, with a visible failure alert, not a generic template pack.
```
