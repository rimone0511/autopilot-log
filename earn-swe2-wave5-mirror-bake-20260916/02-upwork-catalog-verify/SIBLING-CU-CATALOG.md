# Upwork Project Catalog — unpublished DRAFT paste (CU)
...
Aligned with sibling `earn-upwork-catalog-draft-20260916/` (800-word overview lives there). This file is the **CU field map**. If both folders exist, texts should match; **live form wins**.
...
If the create form **requires** prices to proceed and placeholders are empty: fill the **text** fields, then park `rate_required` rather than typing guessed USD. Do not Submit.
...
Working 800-word overview (too long for this box): sibling `earn-upwork-catalog-draft-20260916/overview-en-800.md` if present.
...
Do not ask for live credentials in this form.
...
Do not attach Autopilot Log as if this Catalog item were that CLI.

CU this session: **skip gallery** if it is required only to Submit. Empty gallery + no Submit is the intended unpublished state.
...
Verdict: **needs_check** for Catalog browse from restricted environments; marketplace product is **not treated as closed**.

Evidence (2026-09-15), no traffic totals invented:

- Upwork help articles for creating, reviewing, and writing Catalog projects returned live content (sibling Catalog pack).
- `https://www.upwork.com/catalog` from some environments presented a JavaScript / verification interstitial, so listing freshness was **not** counted (same class of block as a Cloudflare wall: not proof the desk is dead).

Do not write GMV, job counts, or “Connects required to list.” Confirm at click-time whether Create Project is visible — and still do not submit. **0 Connects.**
