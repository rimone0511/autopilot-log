...
Snapshot: **2026-09-16 05:19 JST** (folder stamp)  
Live correction: **2026-09-16 05:22 JST** — LinkedIn `blocked_skip` **reCAPTCHA**; TimeTicket `in_progress`  
...
This snapshot **overrides** older “next CU” hints: [PR#24](https://github.com/rimone0511/autopilot-log/pull/24) CrowdWorks-next, [PR#35](https://github.com/rimone0511/autopilot-log/pull/35) Upwork-next, [PR#43](https://github.com/rimone0511/autopilot-log/pull/43) Upwork-pending, and this file’s first revision (LinkedIn `in_progress` profile-only). QUEUE `done-draft` / `next` rows are not rewritten here.
...
| A6 / CU-06 | LinkedIn | `blocked_skip` | **reCAPTCHA** | [#16](https://github.com/rimone0511/autopilot-log/pull/16) | Park. Do not retry reCAPTCHA. Do not Save a Service Page. Jobs / feed out of scope |
| A7 / CU-07 | TimeTicket | `in_progress` | **current CU** | [#18](https://github.com/rimone0511/autopilot-log/pull/18) | Open desk. Host + ticket **draft**; prefer message (async); no 発行完了. Do not signup |
...
Counts this box: `draft_saved` 2 · `blocked_skip` 5 · `in_progress` 1 · `pending` 3.

## This pass

1. Skip Fiverr (`hold`), Lancers (`captcha`), CrowdWorks (`403`), Upwork (`Google SSO`), LinkedIn (`reCAPTCHA`). No retry.
2. Open desk is **TimeTicket** (A7 / current CU). Host + ticket draft only. Not 発行完了. Not signup.
3. After TimeTicket parks: **Contra → Craudia → Freelancer.com**.
